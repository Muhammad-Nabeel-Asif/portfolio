...
export default function ExperienceTimeline() {
  ...
  return (
    <div className="bg-zinc-950 rounded-2xl border border-zinc-900 shadow-xl p-4 sm:p-6">
      <div className="border-b border-zinc-900 pb-5 mb-8">
        ...
        <p className="text-xs text-zinc-400 font-sans mt-0.5">Filter work terms to audit technical contributions, corporate impact outcomes, and project portfolios.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        
        {/* Left column: Company selectors (horizontal scroll on mobile, vertical list on desktop) */}
        <div className="lg:col-span-4 flex flex-row lg:flex-col overflow-x-auto lg:overflow-x-visible pb-4 lg:pb-0 gap-2 border-b lg:border-b-0 lg:border-r border-zinc-900 lg:pr-4 scrollbar-thin" aria-label="Work Chronology">
            ...
            return (
              <button
                ...
                className={`cursor-pointer shrink-0 lg:shrink w-[200px] sm:w-[240px] lg:w-full text-left p-3.5 rounded-xl border transition-all duration-150 relative ${
                  isActive
                    ? 'bg-zinc-900 border-zinc-805 text-white shadow-md ring-1 ring-zinc-850'
              ...
              >
                ...
                <h4 className="text-xs font-bold text-white mt-1 leading-tight truncate">{company.name}</h4>
                <p className="text-3xs font-mono text-emerald-400/90 mt-1 truncate">{company.role}</p>
              </button>
            );
          })}
        </div>
        ...
        <div className="lg:col-span-8 border border-zinc-900 rounded-2xl p-4 sm:p-6 bg-zinc-900/10 min-h-[460px] flex flex-col justify-between">
          
          <div>
            {/* Header info */}
            <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-zinc-900 pb-4 mb-6 gap-3">
              <div>
                <h3 className="text-base font-bold text-white tracking-tight">{selectedCompany.role}</h3>
                <p className="text-3xs font-mono text-emerald-400 mt-1 uppercase">{selectedCompany.name}</p>
              </div>
              ...
              <ul className="space-y-4" aria-label="Technical Achievements">
                {selectedCompany.achievements.map((item, index) => (
                  <li key={index} className="flex items-start space-x-3 group">
                    ...
                    <span className="text-xs text-zinc-300 leading-relaxed font-sans">{item}</span>
                  </li>
                ))}
              </ul>
            )}
            ...
            {activeTab === 'projects' && selectedCompany.keyProjects && (
              <div className="space-y-6" aria-label="Projects Repository">
                {selectedCompany.keyProjects.map((project: Project) => (
                  <div key={project.id} className="border border-zinc-900 rounded-xl bg-zinc-950 p-4 sm:p-5 shadow-inner">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-zinc-900/60 pb-3 mb-4 gap-2">
                      <div>
                      ...
                      <span className="text-4xs font-mono text-emerald-400/90 uppercase border border-emerald-500/15 rounded bg-emerald-500/5 px-2 py-0.5 self-start md:self-auto">
                        {project.role}
                      </span>
                    </div>

                    <p className="text-2xs text-zinc-400 font-sans leading-normal mb-4">
                      {project.description}
                    </p>
                      ...
                      <div className="my-4 border-l-2 border-emerald-500/40 pl-3 py-1 bg-emerald-500/5 rounded-r">
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1">
                          {project.impactMetrics.map((kpi, idx) => (
                            <span key={idx} className="inline-flex items-center text-3xs font-mono text-emerald-400">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-4">
                      {project.techStack.map((tech) => (
                        <span key={tech} className="text-4xs font-mono text-zinc-405 border border-zinc-900 rounded bg-zinc-900/30 px-1.5 py-0.5">
                          {tech}
                        </span>
                      ))}
                    </div>
          ...
          <div className="border-t border-zinc-900/80 pt-5 mt-8">
            ...
            <div className="flex flex-wrap gap-1.5">
              {selectedCompany.coreTech.map((tech) => (
                <span key={tech} className="text-4xs font-mono text-emerald-400/80 border border-zinc-900 rounded-lg bg-zinc-950 px-2 py-1 select-all">
                  {tech}
                </span>
              ))}
            </div>
